...
# Load sample data from .claude directory
@pytest.fixture
def sample_threads_data():
    """Load sample threads data from the .claude directory."""
    sample_path = os.path.join(
        os.path.dirname(__file__),
        "..",
        ".claude",
        "v1_community_publications_4906951_posts.json",
    )
...
def sample_comments_data():
    """Load sample comments data from the .claude directory."""
    sample_path = os.path.join(
        os.path.dirname(__file__),
        "..",
        ".claude",
        "v1_community_posts_4e1f8bb1-cea6-4bc9-889d-e8856df852d0_comments.json",
    )
    if os.path.exists(sample_path):
        with open(sample_path) as f:
            return json.load(f)
...
class TestChat:
    ...
    def test_init_with_string_publication_id(self, mock_auth):
        """Test Chat initialization with string publication_id."""
        chat = Chat(publication_id="4906951", auth=mock_auth)
        assert chat.publication_id == 4906951
